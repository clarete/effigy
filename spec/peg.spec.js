...
describe("List matcher", () => {
  it("should work in tandem with char matcher", () => {
    ...
    const pg = pegc(
      ...
      'N <- [0-9]+    \n' +
      'P <- "+"       \n');
    const bpg = pg.bind(parseActions);
    // Grammar for parsing lists with the shape of the parse tree
    // generated by the above grammar
    const tg = pegc(
      'T <- { V { { "+" V }* } }    \n' +
      'V <- !{ .* } .               \n');

    // When some input is matched against the pipeline of the two
    // grammars
    const matched = tg.matchl(bpg('12+345+8'));

    // Then it should return the following parsed structure
